...
function Workspace({ initial }) {
  ...
  const send = useCallback((text, image, voice = false, retryMessage = null) => {
    ...
    const payload = { type: 'text', content: text.trim(), uploaded_image: image || null, chat_session_id: session.id, session_id: turnId, voice_mode: voice || dataRef.current.settings.voiceReplies, language: dataRef.current.settings.language }
